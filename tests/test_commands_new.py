...
class TestCmdThink:
    async def test_show_current(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session)
        await cmd_think(ctx)
    ...
    async def test_think_on(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["on"])
        await cmd_think(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_think_on_custom_budget(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["on", "50000"])
        await cmd_think(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_think_on_invalid_budget(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["on", "notanumber"])
        await cmd_think(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_think_off(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["off"])
        await cmd_think(ctx)
    ...
    async def test_think_adaptive(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["adaptive"])
        await cmd_think(ctx)
    ...
    async def test_think_invalid_mode(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["bogus"])
        await cmd_think(ctx)
        text = ctx.reply.call_args[0][0]
...
class TestCmdEffort:
    ...
    async def test_set_high(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["high"])
        await cmd_effort(ctx)
    ...
    async def test_set_max(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["max"])
        await cmd_effort(ctx)
    ...
    async def test_set_low(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["low"])
        await cmd_effort(ctx)
    ...
    async def test_effort_invalid(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["extreme"])
        await cmd_effort(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
...
class TestCmdUsage:
    ...
    async def test_with_usage_data(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            bot_data={"usage": {sm_with_session.current.name: su}},
        )
        await cmd_usage(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_usage_all(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["all"])
        await cmd_usage(ctx)
        text = ctx.reply.call_args[0][0]
...
class TestCmdModel:
    ...
    async def test_set_sonnet(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["sonnet"])
        await cmd_model(ctx)
    ...
    async def test_set_opus(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["opus"])
        await cmd_model(ctx)
    ...
    async def test_set_haiku(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["haiku"])
        await cmd_model(ctx)
    ...
    async def test_set_full_model_name(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["claude-sonnet-4-20250514"])
        await cmd_model(ctx)
        assert sm_with_session.current.model == "claude-sonnet-4-20250514"
    ...
    async def test_model_default(self, sm_with_session):
        ...
        sm_with_session.current.model = "claude-opus-4-6"
        ctx = MockTransport(session_manager=sm_with_session, args=["default"])
        await cmd_model(ctx)
        assert sm_with_session.current.model is None
...
class TestCmdHistory:
    ...
    async def test_history_search(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["search", "fox"])
        await cmd_history(ctx)
        text = ctx.reply.call_args[0][0]
        assert "fox" in text
    ...
    async def test_history_search_no_results(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["search", "nonexistent"])
        await cmd_history(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_history_stats(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["stats"])
        await cmd_history(ctx)
        text = ctx.reply.call_args[0][0]
...
class TestHistorySearchWithResults:
    ...
    async def test_history_search_returns_results(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["search", "widget"])
        await cmd_history(ctx)
        text = ctx.reply.call_args[0][0]
...
class TestCmdAutonomous:
    ...
    async def test_set_turns(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["turns", "100"])
        await cmd_autonomous(ctx)
    ...
    async def test_set_turns_invalid(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["turns", "abc"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_set_turns_too_low(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["turns", "0"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_show_turns_no_value(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["turns"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_set_budget(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["budget", "2.50"])
        await cmd_autonomous(ctx)
    ...
    async def test_budget_off(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["budget", "off"])
        await cmd_autonomous(ctx)
    ...
    async def test_budget_invalid(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["budget", "xyz"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_budget_negative(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["budget", "-5"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_show_budget_no_arg(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["budget"])
        await cmd_autonomous(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_on_aliases(self, sm_with_session):
        ...
        for arg in ("true", "1"):
            ...
            ctx = MockTransport(session_manager=sm_with_session, args=[arg])
            await cmd_autonomous(ctx)
...
class TestCmdCron:
    ...
    async def test_add_job_too_few_args(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["add", "morning"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_remove_job(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["remove", "temp"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_remove_not_found(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["remove", "nope"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_remove_no_name(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["remove"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_pause_job(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["pause", "daily"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_pause_not_found(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["pause", "nope"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "not found" in text
    ...
    async def test_pause_no_name(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["pause"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_resume_job(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["resume", "daily"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_resume_not_found(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["resume", "nope"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "not found" in text
    ...
    async def test_resume_no_name(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["resume"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Usage" in text
    ...
    async def test_remove_db_failure(self, mock_gs, sm_with_session):
        ...
        mock_gs.side_effect = Exception("DB error")
        ctx = MockTransport(session_manager=sm_with_session, args=["remove", "something"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_pause_db_failure(self, mock_gs, sm_with_session):
        ...
        mock_gs.side_effect = Exception("DB error")
        ctx = MockTransport(session_manager=sm_with_session, args=["pause", "something"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Failed" in text
    ...
    async def test_resume_db_failure(self, mock_gs, sm_with_session):
        ...
        mock_gs.side_effect = Exception("DB error")
        ctx = MockTransport(session_manager=sm_with_session, args=["resume", "something"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Failed" in text
    ...
    async def test_delete_alias(self, sm_with_session):
        ...
        ctx = MockTransport(session_manager=sm_with_session, args=["delete", "tmp"])
        await cmd_cron(ctx)
        text = ctx.reply.call_args[0][0]
        assert "Deleted" in text
...
class TestCmdHeartbeat:
    async def test_status_stopped_no_checks(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            bot_data={"scheduler": None, "config": None},
        )
        await cmd_heartbeat(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_status_running_with_checks(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            bot_data={"scheduler": mock_scheduler, "config": None},
        )
        await cmd_heartbeat(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_add_check(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            args=["add", "disk", "Check", "disk", "usage"],
            bot_data={"scheduler": None, "config": None},
        )
        await cmd_heartbeat(ctx)
        text = ctx.reply.call_args[0][0]
    ...
    async def test_start(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            args=["on"],
            bot_data={"scheduler": None, "config": None, "_bot": AsyncMock()},
        )
        await cmd_heartbeat(ctx)
    ...
    async def test_start_stops_existing(self, sm_with_session):
        ...
        ctx = MockTransport(
            session_manager=sm_with_session,
            args=["on"],
            bot_data={"scheduler": existing, "config": None, "_bot": AsyncMock()},
        )
        await cmd_heartbeat(ctx)
